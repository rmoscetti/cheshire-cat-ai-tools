...
# %% auto 0
__all__ = ['settings', 'conf_variants', 'hallucination_scorer', 'similarity_scorer', 'CatModel', 'load_eval_dataset',
           'repeat_dataset', 'CatEmbeddingSimilarityScorer', 'eval_configs']
...
async def eval_configs(dataset, n_rep=3):
    evaluation = weave.Evaluation(
        dataset=list(repeat_dataset(dataset, n_rep)),
        scorers=[hallucination_scorer, similarity_scorer],
    )
    for name, conf in tqdm(conf_variants.items(), total=len(conf_variants)):
